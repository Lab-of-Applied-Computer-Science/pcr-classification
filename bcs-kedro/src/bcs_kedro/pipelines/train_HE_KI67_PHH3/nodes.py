...
# PARA USAR COM DADOS VIRTUAIS updated parquet
# @tf.function
# def preprocess_path(path, is_hes=False):
#     if is_hes:
#         path = tf.strings.regex_replace(path, "PHH3", "HES")
#         path = tf.strings.regex_replace(path, "/scratch/henrique.colonese/breast-cancer-segmentation/bcs-kedro/data/03_primary/", "/home_cerberus/speed/daniel.ayala/breast-cancer-segmentation/bcs-kedro/data/02_intermediate/spatial-attention-pcr/")
#         # path = tf.strings.regex_replace(tf.strings.regex_replace(path, "03_primary", "02_intermediate"), "-deconv", "")
#     else:
#         path = tf.strings.regex_replace(path, "03_primary/", "03_primary_VS/")
#     return path
...
def process_row(row):
    ...
    try:
        ...
        # Concatenate results
        combined = tf.concat([hes, ki67, phh3], axis=-1)
...
def load_split_datasets():
    ...
    train_df = pd.read_csv("/scratch/henrique.colonese/breast-cancer-segmentation/data/05_model_input/data_splits/train_data.csv")
    # val_df = pd.read_csv("data_splits/val_data.csv")
    val_df = pd.read_csv("/scratch/henrique.colonese/breast-cancer-segmentation/data/05_model_input/data_splits/val_data.csv")
...
def build_dataset_with_filenames(df, batch_size, filename_col="path_ki67"):
    ...
    ds = ds_rows.map(_map, num_parallel_calls=tf.data.AUTOTUNE)
    ds = ds.batch(batch_size).prefetch(tf.data.AUTOTUNE)
    return ds
